  ...
  return (
    <div className="border-t border-slate-200 bg-white p-4">
      ...
      {attachments.length > 0 && (
        <div className="flex gap-2 mb-3 overflow-x-auto pb-2">
          {attachments.map((att, idx) => (
            <div
              key={idx}
              className="relative flex items-center gap-2 bg-slate-100 border border-slate-200 rounded-lg px-3 py-2 min-w-[150px] max-w-[250px]"
            >
              ...
              <span className="text-xs text-slate-700 truncate font-medium flex-1">
                {att.name}
              </span>
              <button
      ...
      <div className="flex items-end gap-2 bg-slate-50 border border-slate-300 rounded-2xl p-2 focus-within:ring-2 focus-within:ring-teal-500 focus-within:border-transparent transition-all shadow-sm">
        ...
        <textarea
          ...
          placeholder={
            attachments.length > 0
              ? "Tambahkan pesan..."
              : "Tanyakan tentang kenaikan pangkat, cuti, atau permohonan informasi..."
          }
          disabled={disabled}
      ...
      <p className="text-[10px] text-slate-400 mt-2 text-center">
        AI dapat membuat kesalahan. Periksa kembali informasi dengan dokumen
        resmi.
      </p>
    </div>
  );
